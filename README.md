...
# ssm421课程辅导网站的设计与实现+jsp

![picture](https://raw.githubusercontent.com/GraduationProject-springboot/.github/main/img/wx.png)

### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=20)


# 绪论
## 1.1 研究背景
现在大家正处于互联网加的时代，这个时代它就是一个信息内容无比丰富，信息处理与管理变得越加高效的网络化的时代，这个时代让大家的生活不仅变得更加地便利化，也让时间变得更加地宝贵化，因为每天的每分钟，每秒钟这些时间都能让人们处理大批量的日常事务，这些场景，是之前的手工模式无法与之相抗衡的。对于课程辅导信息的管理来说，传统的通过纸质文档记录信息的方式已经落后了，依靠手工管理这些信息，不仅花费较长的工作时间，在对记录各种信息的文档进行信息统计以及信息核对操作时，也不能及时保证信息的准确性，基于这样的办公低效率环境下，对于课程辅导信息的处理就要提出新的解决方案。因为这个时代的信息一直都在高速发展，要是不抱着发展的观念看待事情，极有可能被这个市场快速遗忘，甚至被无情地淘汰掉。所以尽早开发一款课程辅导网站进行信息的快速处理，既跟上了时代的发展脚步，也能让自己的核心竞争力有所提升。
## 1.2目的和意义
互联网加的时代一方面是加快信息的发展，另一方面也是对传统行业进行筛选，能够继续发展的，肯定是那些能够充分运用互联网技术进行自身升级改革的行业。那些停步不前的行业只能就此结束，进而被大家所遗忘。这次设计出来的课程辅导网站，它不仅能够让管理人员在信息增加，信息的编辑等事务处理上，节省很多的时间，也会砍掉一部分的人工成本，节省不必要开支的资金。另外，此系统的操作界面是可视化的界面，管理人员无需付费培训就能尽快上手。课程辅导网站的开发意义如下：
...
4、课程辅导信息都是通过网站进行显示，其实质是这些信息都保存在网站对应的数据库里面。只要操作员不去恶意删除信息，那么这些信息将会永久保存。
## 1.3 论文结构安排
编写课程辅导网站相对应的论文，其实就是对开发完成的程序进行再次解读的过程。本论文从七个方面的内容讲解了开发的程序，具体内容如下：
...
# 3 系统分析
## 3.1系统可行性分析
需要使用大部分精力开发的课程辅导网站为了充分降低开发风险，特意在开发之前进行可行性分析这个验证系统开发是否可行的步骤。本文就会从技术角度，经济角度，还有用户使用的程序的运行角度进行综合阐述。
### 3.1.1 技术可行性分析
...
## 3.3系统流程分析
管理员假如要操作系统提供的功能，那么管理员就要在系统的登录界面，填写管理员登录的账号信息，填写相应的密码信息，管理员需要保证这两者能够验证身份的账号以及密码信息的正确性，这样管理员就可以通过登录界面进入系统后台操作界面。图3.1就是开发的程序软件课程辅导网站它的操作流程图。

![](/md/blog.001.png)

图3.1 系统操作流程图
### 3.3.1系统登录流程
课程辅导网站的登录流程，针对的角色就是操作员的操作角色。在登录界面需要的必填信息就是账号信息，配上登录的密码信息就能登录课程辅导网站，需要注意的就是必填的账号信息和登录密码信息，都需要进行验证，系统会判断账号还有填写的密码信息的正确性，只有这两者信息都正确了，就能成功登录课程辅导网站了。系统登录流程图如下图。
...
# 4 系统设计
## 4.1系统概要设计
课程辅导网站并没有使用C/S结构，而是基于网络浏览器的方式去访问服务器，进而获取需要的数据信息，这种依靠浏览器进行数据访问的模式就是现在用得比较广泛的适用于广域网并且没有网速限制要求的B/S结构，图4.1就是开发出来的程序工作原理图。
...
|jiaoshi\_name|varchar(200)|是|NULL|教师姓名 Search111|
|sex\_types|int(11)|是|NULL|性别 Search111|
|jiaoshi\_id\_number|varchar(200)|是|NULL|身份证号|
|jiaoshi\_phone|varchar(200)|是|NULL|手机号|
|jiaoshi\_email|varchar(200)|是|NULL|邮箱|
...
| :-: | :-: | :-: | :-: | :-: |
|id (主键)|int(11)|否||主键|
|jiaoxueziyuan\_name|varchar(200)|是|NULL|教学资源名称 Search111|
|jiaoxueziyuan\_types|int(200)|是|NULL|资源类型 Search111|
|zan\_number|int(200)|是|NULL|赞|
|cai\_number|int(200)|是|NULL|踩|
|jiaoxueziyuan\_photo|varchar(200)|是|NULL|封面|
|jiaoxueziyuan\_file|varchar(200)|是|NULL|教学资源下载|
|jiaoxueziyuan\_content|text|是|NULL|教学资源详情|
...
| :-: | :-: | :-: | :-: | :-: |
|id (主键)|int(11)|否||主键|
|jiaoxueziyuan\_id|int(11)|是|NULL|教学资源|
...
|jiaoxueziyuan\_id|int(11)|是|NULL|教学资源|
|yonghu\_id|int(11)|是|NULL|用户|
|jiaoxueziyuan\_liuyan\_content|text|是|NULL|评价内容|
|reply\_content|text|是|NULL|回复内容|
|insert\_time|timestamp|是|NULL|评价时间|
|create\_time|timestamp|是|NULL|创建时间|
表4.5 推荐课程信息表
...
| :-: | :-: | :-: | :-: | :-: |
|id (主键)|int(11)|否||主键|
|kecheng\_name|varchar(200)|是|NULL|课程名称 Search111|
|kecheng\_types|int(200)|是|NULL|课程类型 Search111|
|zan\_number|int(200)|是|NULL|赞|
|cai\_number|int(200)|是|NULL|踩|
|kecheng\_photo|varchar(200)|是|NULL|封面|
|kecheng\_video|varchar(200)|是|NULL|课程视频|
|kecheng\_content|text|是|NULL|课程详情|
|create\_time|timestamp|是|NULL|创建时间 show1 show2 photoShow|
表4.6 课程收藏信息表
...
|kecheng\_id|int(11)|是|NULL|课程|
|yonghu\_id|int(11)|是|NULL|用户|
|kecheng\_liuyan\_content|text|是|NULL|评价内容|
|reply\_content|text|是|NULL|回复内容|
|insert\_time|timestamp|是|NULL|评价时间|
|create\_time|timestamp|是|NULL|创建时间|
表4.8 公告信息表
...
|username|varchar(200)|是|NULL|账户|
|password|varchar(200)|是|NULL|密码|
|yonghu\_name|varchar(200)|是|NULL|学生姓名 Search111|
|sex\_types|int(11)|是|NULL|性别 Search111|
...
# 5 系统实现
## 5.1前台首页
学生必须登录才可以看到首页信息，首页信息主要包含导航栏，轮播图，公告信息等。下图就是前台首页页面。

![](/md/blog.011.png)

图5.1 前台首页页面
## 5.2 教学资源
用户登录后可以在首页导航界面点击教学资源可以查看教学资源列表信息。下图就是教学资源页面。

![](/md/blog.012.png)

图5.2教学资源页面
## 5.3 推荐课程
用户登录后可以查看推荐课程。下图就是推荐课程页面。

![](/md/blog.013.png)

图5.3 推荐课程页面
## 5.4我的
学生用户登录后在前台导航点击我的可以看到子菜单，分别是个人中心，教学资源收藏，课程收藏，点击不同的导航可以跳转到不同的界面。下图就是我的页面。

![](/md/blog.014.png)

图5.4 我的页面
## 5.5教师信息管理
管理员可以对教师信息进行添加，修改，删除，查询操作。下图就是教师信息管理页面。

![](/md/blog.015.png)

图5.5 教师信息管理页面
## 5.6学生信息管理
管理员可以对学生信息进行添加，修改，删除，查询操作。下图就是学生信息管理页面。

![](/md/blog.016.png)

图5.6 学生信息管理页面
## 5.7教学资源管理
管理员和教师都可以对教学资源信息进行添加，修改，删除，查询操作。下图就是教学资源管理页面。

![](/md/blog.017.png)

图5.7 教学资源管理页面
## 5.8推荐课程管理
管理员和教师可以对推荐课程信息进行添加，修改，删除，查询操作。下图就是推荐课程管理页面。

![](/md/blog.018.png)

图5.8 推荐课程管理页面
## 5.9轮播图管理
管理员对轮播图进行管理，轮播图主要是显示在首页导航栏下面。下图就是轮播图管理页面。

![](/md/blog.019.png)

图5.9 轮播图管理页面
## 5.10公告信息管理
管理员可以对公告信息进行添加，修改，删除，查询操作。下图就是公告信息管理页面。
